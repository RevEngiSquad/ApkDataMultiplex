...
public class DataMultiplexing {
     ...
     */
    public static void optimize(String input, String output, String hostEntryName, boolean printDetails)
            throws IOException {
     ...
     * @param printDetails  是否打印优化详情
     */
    public static void optimize(File input, File output, String hostEntryName, boolean printDetails)
            throws IOException {
        try (ZipFile zipFile = new ZipFile(input)) {
            ...
            try (ZipMaker zipMaker = new ZipMaker(output)) {
                ...
                if (printDetails) {
                    System.out.println(hostEntry.getName() + " >> offset=0x"
                            + Long.toHexString(holder.getHostEntryHeaderOffset()));
                }
        ...
        System.out.printf("Data multiplexing optimize: %s (%s) -> %s (%s)  [%.2f%%]\n", input.getName(),
                formatFileSize(inputLen), output.getName(), formatFileSize(outputLen),
                (outputLen - inputLen) * 100f / inputLen);
    }
        ...
        try (ZipFile inner = openEntryAsZipFile(outer, hostEntry)) {
            for (ZipEntry outerEntry : outer.getEntries()) {
                ...
                boolean equals = outerEntry.getCompressedSize() == innerEntry.getCompressedSize() &&
                        isInputStreamContentEquals(inner.getRawInputStream(innerEntry),
                                outer.getRawInputStream(outerEntry))
                        ||
