...
package org.revengi.apksign;

import org.revengi.apksign.data.DataSink;
import org.revengi.apksign.data.DataSinks;
import org.revengi.apksign.data.DataSource;
import org.revengi.apksign.data.DataSources;
...
/**
 * VerityTreeBuilder is used to generate the root hash of verity tree built from
 * the input file.
 * The root hash can be used on device for on-access verification. The tree
...
class VerityTreeBuilder {
     ...
     * <p>
     * Specifically, APK verity tree is built from the APK, but as if the APK
     * Signing Block (which
     * must be page aligned) and the "Central Directory offset" field in End of
     * Central Directory
     * are skipped.
     */
     ...
     * <p>
     * The tree is built bottom up. The bottom level has 256-bit digest for each 4
     * KB block in the
     * input file. If the total size is larger than 4 KB, take this level as input
     * and repeat the
     * same procedure, until the level is within 4 KB. If salt is given, it will
     * apply to each
     * digestion before the actual data.
     * <p>
     * The returned root hash is calculated from the last level of 4 KB chunk,
     * similarly with salt.
     * <p>
     * The tree is currently stored only in memory and is never written out.
     * Nevertheless, it is
     * the actual verity tree format on disk, and is supposed to be re-generated on
     * device.
     * <p>
    ...
    private byte[] generateVerityTreeRootHash(DataSource fileSource) throws IOException {
        ...
        // Calculate the summed area table of level size. In other word, this is the
        // offset
    ...
    /**
     * Returns an array of summed area table of level size in the verity tree. In
     * other words, the
     * returned array is offset of each level in the verity tree file format, plus
     * an additional
     * offset of the next non-existing level (i.e. end of the last level + 1). Thus
     * the array size
     * is level + 1.
     */
    ...
    /**
     * Digest data source by chunks then feeds them to the sink one by one. If the
     * last unit is
     * less than the chunk size and padding is desired, feed with extra padding 0 to
     * fill up the
